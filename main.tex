\documentclass[12pt,a4paper,2sides]{report}
\usepackage{lib/lib} %import libary from lib.sty
\usepackage{minitoc}
%Thêm dữ liệu vào intro.tex qua dòng lệnh.
%Addition into data at intro.tex file via command.
<<<<<<< HEAD
\newcommand{\bai}{ĐỒ ÁN CUỐI KÌ}
\newcommand{\de}{QUẢN LÝ CỬA HÀNG TIỆC CƯỚI
	\\ \small{PHÁT TRIỂN HỆ THỐNG THÔNG TIN DOANH NGHIỆP}}
\newcommand{\gvhd}{ThS. Dương Hữu Phúc}
\newcommand{\tacgia}{Nguyễn Dương}
\newcommand{\mstacgia}{51702012}
\newcommand{\svhai}{Lê Minh Hiếu}
\newcommand{\msvhai}{51702016}
=======
\newcommand{\khoa}{KHOA CÔNG NGHỆ THÔNG TIN}
\newcommand{\bai}{Sản phẩm}
\newcommand{\de}{Tên đề tài
	\\ \small{MÔN HỌC}}
\newcommand{\gvhd}{Giảng viên hướng dẫn}
\newcommand{\tacgia}{Tác giả}
\newcommand{\mstacgia}{Mã tác giả}
\newcommand{\svhai}{}
\newcommand{\msvhai}{}
>>>>>>> fe34056a9bdc662673fefe215a024ac587cbb65f
\newcommand{\svba}{}
\newcommand{\msvba}{}
\newcommand{\svtu}{}
\newcommand{\msvtu}{}
\newcommand{\nam}{2020}
%Acknowledge page (the second page in docs)
\newcommand{\loicamon}{
		Với những kiến thức em và các bạn cùng nhóm đã tích lũy được qua những ngày học tập, đây là kết quả của quá trình học tập của nhóm cùng đưa ra ý kiến và thảo luận, tổng hợp kiến thức lại với nhau.\\
}
%tabl
\begin{document}	
\pagenumbering{roman}
\input{intro.tex} #Nhập file intro.tex, chuột phải để mở chúng lên.
\newpage
\clearpage
%--------------Mục lục
\dominitoc
\tableofcontents %pages
%if you want to show table of contents of chapter, set \dominitoc under line \chapter{name}
%\listoffigures %pictures
\newpage
\clearpage
%\setcounter{page}{-1}
%\listoftables
%\begin{figure/tables}
%\centering
%Sourcode/Import
%\caption
%\end{fugure/tables}
\clearpage
\pagenumbering{arabic}
\setcounter{page}{1}
\clearpage
%\Control to file to create subfile.\\
\input{part1.tex}
%\input{part2.tex}
%\input{part3.tex}
\begin{thebibliography}{5}
	\bibitem{}  Tài liệu học tập trường đại học Tôn Đức Thắng.
	\bibitem{}
	\bibitem{}
\end{thebibliography}
\end{document}
